...
<template>
  <main class="reviews-page min-h-[calc(100dvh-var(--mobile-dock-height))] bg-[#f8fbff] px-6 py-12 text-ink sm:px-10 sm:py-14 desktop:min-h-dvh desktop:px-page desktop:py-16">
    ...
    <section class="mx-auto mt-8 max-w-[96rem] sm:mt-10" aria-label="Видеоотзывы клиентов">
      <template v-if="reviews.items.length">
        <div class="grid gap-8 md:grid-cols-2 desktop:grid-cols-3 xl:gap-10 2xl:gap-12">
          <article
            ...
            :key="review.id"
            class="relative min-w-0 overflow-hidden border border-ink/10 bg-white shadow-[0_1.25rem_3.5rem_rgb(7_16_31/0.07)]"
          ...
          >
            ...
            <div
              ...
              class="absolute inset-0 z-10 animate-pulse bg-white"
              aria-hidden="true"
            >
              <div class="aspect-video w-full bg-[#e8f1ff]" />
              <div class="border-t border-ink/10 p-5 sm:p-6">
                <div class="h-6 w-3/5 bg-[#dbe9fb]" />
                <div class="mt-3 h-4 w-4/5 bg-[#edf4fd]" />
                <div class="mt-2 h-4 w-2/5 bg-[#edf4fd]" />
              </div>
            </div>
          </article>
        </div>
      ...
      <div v-else class="grid min-h-64 place-items-center border border-ink/10 bg-white px-6 text-center text-sm text-muted">
